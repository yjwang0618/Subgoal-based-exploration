...
#============================================================================================#
#||========================================================================================||#
#||                            the parent class for grid-world                             ||#
#||========================================================================================||#
#============================================================================================#
class GridWorld_Parent(Domain):
    ...
    **REWARD:**
    ...
    def __init__(self, noise=.1, episodeCap=1000):
        ...
        super(GridWorld_Parent, self).__init__()
...
#============================================================================================#
#||========================================================================================||#
#||                                grid-world without flags                                ||#
#||========================================================================================||#
#============================================================================================#
class GridWorld(GridWorld_Parent):
    #: Reward constants
    GOAL_REWARD = +1
    PIT_REWARD = -1
    BLOCK_REWARD = 0 # -0.1
    ...
    def __init__(self, mapname=os.path.join(default_map_dir, "4x5.txt"), noise=.1, episodeCap=1000):
        ...
        super(GridWorld, self).__init__(noise=noise, episodeCap=episodeCap)
...
#||========================================================================================||#
#============================================================================================#
class GridWorld_Flag(GridWorld_Parent):
    #: Reward constants
    GOAL_REWARD = +1
    PIT_REWARD = -1
    BLOCK_REWARD = 0 # -0.1
    ...
    def __init__(self, mapname, noise=.1, episodeCap=1000, 
        ...
        super(GridWorld_Flag, self).__init__(noise=noise, episodeCap=episodeCap)
        ...
        # self.STEP_REWARD = np.zeros((self.ROWS, self.COLS, self.ROWS, self.COLS))
        # for r in range(self.ROWS):
        #     for c in range(self.COLS):
        #         s_pos = np.array([[r, c]])
        #         for nr in range(self.ROWS):
        ...
        #                     exponentNext = np.divide(np.sum(0.5*(ns_pos-self.FlagPos[flag,:])**2, axis=1), self.FlagWid[flag])
        #                     phi_ns = np.multiply(self.FlagHeight[flag], np.exp(-exponentNext))
        #                     self.STEP_REWARD[r,c,nr,nc] += self.discount_factor * phi_ns - phi_s
    ...
    #     r = np.ones((k, 1))
    #     for i in range(k):
    #         r[i] = self.STEP_REWARD[s[0], s[1], ns[i,0], ns[i,1], self.collectedFlag]
    #     goal = self.map[ns[:,0].astype(np.int), ns[:,1].astype(np.int)] == self.GOAL
    #     pit = self.map[ns[:,0].astype(np.int), ns[:,1].astype(np.int)] == self.PIT
