...
class GridWorld_Items(Domain):
    ...
    **REWARD:**
    ...
    EMPTY, BLOCKED, START, GOAL, PIT, AGENT, SUBGOAL1, SUBGOAL2 = list(range(8))
    #: Up, Down, Left, Right
    ...
    def __init__(self, mapname=os.path.join(default_map_dir, "4x5.txt"), noise=.1, episodeCap=1000, items_pos=None):
        ...
        self.NOISE = noise # Movement Noise
        ...
        if self.items_pos is not None:
            self.item_collected = np.zeros(self.items_pos.shape[0])
        super(GridWorld_Items, self).__init__()
    ...
    def showLearning(self, representation):
        if self.valueFunction_fig is None:
            ...
            self.valueFunction_fig = plt.imshow(self.map, cmap='ValueFunction', interpolation='nearest', 
                                                vmin=self.MIN_RETURN, vmax=self.MAX_RETURN)
        ...
        with open('GridWorld_Value.txt', 'a') as outfile:
    ...
    def step(self, a):
        ...
        if (ns[0] < 0 or ns[0] == self.ROWS or 
            ...
            ns = self.state.copy()
            r = self.BLOCK_REWARD + self.STEP_REWARD
        else:
        ...
        if self.items_pos is not None:
            ...
            for item_pos in self.items_pos:
                ...
                if ( np.absolute(ns[0]-item_pos[0])<=0.5 and \
                        ...
                        self.item_collected[count]<0.5 ):
                    self.item_collected[count] = 1
...
class GridWorld_Items_Flag(GridWorld_Items):
    def __init__(self, mapname, noise=.1, episodeCap=1000, items_pos=None, 
                 FlagPos=np.array([[0,0]]), 
                 FlagWid=np.array([[1]]), 
                 FlagHeight=np.array([[1]])):     
        super(GridWorld_Items_Flag, self).__init__(mapname=mapname, noise=noise, episodeCap=episodeCap, items_pos=items_pos)
        ...
        STEP_REWARD = np.zeros((self.ROWS, self.COLS, self.ROWS, self.COLS, self.FlagNum+1))
                ...
                for c in range(self.COLS):
                    ...
                    for nr in range(self.ROWS):
                        for nc in range(self.COLS):
                            ...
                            STEP_REWARD[r,c,nr,nc,flag] = self.discount_factor * phi_ns - phi_s
                            self.REWARD[r,c,nr,nc,flag] = STEP_REWARD[r,c,nr,nc,flag]
                            if self.map[nr, nc] == self.GOAL:
                                self.REWARD[r,c,nr,nc,flag] = self.GOAL_REWARD + STEP_REWARD[r,c,nr,nc,flag]
                            if self.map[nr, nc] == self.PIT:
                                self.REWARD[r,c,nr,nc,flag] = self.PIT_REWARD + STEP_REWARD[r,c,nr,nc,flag]
        flag = self.FlagNum
        for r in range(self.ROWS):
            for c in range(self.COLS):
                s = np.array([[r, c]])
    ...
    def step(self, a):
        ...
        if self.items_pos is not None:
            ...
            for item_pos in self.items_pos:
                count += 1
                if np.absolute(ns[0]-item_pos[0])<=0.5 and np.absolute(ns[1]-item_pos[1])<=0.5 and self.item_collected[count]<0.5:
                    self.item_collected[count] = 1
                    r += self.SUBGOAL_REWARD
                    break
        # ------------------------------------ #
        ...
        # ------------------------------------ #
        if self.collectedFlags < self.FlagNum and \
            np.absolute(ns[0] - self.FlagPos[self.collectedFlags,0]) <= 0.5 and \
            np.absolute(ns[1] - self.FlagPos[self.collectedFlags,1]) <= 0.5:
