...
class GridWorld(Domain):
    ...
    **REWARD:**
    ...
    def __init__(self, mapname=os.path.join(default_map_dir, "4x5.txt"), noise=.1, episodeCap=1000):
...
class GridWorld_Flag(GridWorld):
    def __init__(self, mapname, noise=.1, episodeCap=1000, 
                 FlagPos=np.array([[0,0]]), 
                 FlagWid=np.array([[1]]), 
                 FlagHeight=np.array([[1]])):     
        super(GridWorld_Flag, self).__init__(mapname=mapname, noise=noise, episodeCap=episodeCap)
        ...
        # self.REWARD = np.zeros((self.ROWS, self.COLS, self.ROWS, self.COLS))
        # for r in range(self.ROWS):
        #     for c in range(self.COLS):
        #         s = np.array([[r, c]])
        #         for nr in range(self.ROWS):
        ...
        #                 ns = np.array([[nr, nc]])
        #                 if self.map[nr, nc] == self.GOAL:
        #                     self.REWARD[r,c,nr,nc] = self.GOAL_REWARD
        #                 if self.map[nr, nc] == self.PIT:
        #                     self.REWARD[r,c,nr,nc] = self.PIT_REWARD
        ...
        #                     exponentNext = np.divide(np.sum(0.5*(ns-self.FlagPos[flag,:])**2, axis=1), self.FlagWid[flag])
        #                     phi_ns = np.multiply(self.FlagHeight[flag], np.exp(-exponentNext))
        #                     self.REWARD[r,c,nr,nc] += self.discount_factor * phi_ns - phi_s
    ...
    def step(self, a):
        ...
        #     np.absolute(ns[0] - self.FlagPos[self.collectedFlags,0]) <= 1 and \
        #     np.absolute(ns[1] - self.FlagPos[self.collectedFlags,1]) <= 1:
        #     # print(ns, self.FlagPos[self.collectedFlags,:], r, self.collectedFlags)
        #     self.collectedFlags += 1
        # # else: print(ns, r, self.collectedFlags)
        ...
        if self.collectedFlags < self.FlagNum and \
            ...
            self.collectedFlags += 1
            # print(self.collectedFlags, a, s, ns)

        # if self.collectedFlags < self.FlagNum and \
        #     ns[0] == int(self.FlagPos[self.collectedFlags,0]) and \
        #     ns[1] == int(self.FlagPos[self.collectedFlags,1]):
        #     self.collectedFlags += 1
