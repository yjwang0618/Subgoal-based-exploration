...
import logging
from problems.rlpy.Tools import plt
import numpy as np
from copy import deepcopy
import re
import argparse
from problems.rlpy.Tools import deltaT, clock, hhmmss
from problems.rlpy.Tools import className, checkNCreateDirectory
from problems.rlpy.Tools import printClass
import problems.rlpy.Tools.results
import os
...
class Experiment(object):
    ...
    def __init__(self, agent, domain, performance_domain, exp_id=1, max_steps=max_steps,
        ...
        self.domain = domain
        self.performance_domain = performance_domain
    ...
    def seed_components(self):
        ...
        self.output_filename = '{:0>3}-results.txt'.format(self.exp_id)
        ...
        inspect.getfile(self.performance_domain.__class__)
    ...
    def performanceRun(self, total_steps, visualize=False):
        ...
        # print(len( self.agent.representation.weight_vec ))
        # action_num = 4
        # flag_num = 2
        # gw_size = 70
        ...
        #         print(self.agent.representation.weight_vec[aaa*state_size+fff*gw_size : aaa*state_size+fff*gw_size+gw_size])
        ...
        while not eps_term and eps_length < self.domain.episodeCap:
            ...
            s = ns
            eps_return += r
            eps_discount_return += self.performance_domain.discount_factor ** eps_length * r
            eps_length += 1
        if visualize:
            self.performance_domain.showDomain(a)
    ...
    def save(self):
        ...
        ### write
#        writer = csv.writer(open(results_fn, 'w'))    
...
#            writer.writerow([k,v])
    ...
    def load(self):
        """loads the experimental results from the ``results.json`` file
        If the results could not be found, the function returns ``None``
        and the results array otherwise.
        """
        results_fn = os.path.join(self.full_path, self.output_filename)
        self.results = problems.rlpy.Tools.results.load_single(results_fn)
        return self.results
...
# =============================================================================================================== #
#                                class to use when doing the MountainCar problem                                  #
#                                (the flags are turned off after some iterations)                                 #
# =============================================================================================================== #
class Experiment_MountainCar(Experiment):
    """
    ...
    def __init__(self, agent, domain, performance_domain, exp_id=1, max_steps=1000,
                 config_logging=True, num_policy_checks=10, log_interval=1,
                 path='Results/Temp',
                 checks_per_policy=1, stat_bins_per_state_dim=0, **kwargs):
        super(Experiment_MountainCar, self).__init__(agent, domain, performance_domain, exp_id, max_steps,
                                                     config_logging, num_policy_checks, log_interval,
                                                     path,
                                                     checks_per_policy, stat_bins_per_state_dim, **kwargs)
    ...
    def run(self, visualize_performance=0, visualize_learning=False,
        ...
        self.total_eval_time = 0.
        terminal = True
        total_episode_flags, with_flag = 0, True
        while total_steps < self.max_steps:
            if terminal or eps_steps >= self.domain.episodeCap:
                s, terminal, p_actions = self.domain.s0()
                a = self.agent.policy.pi(s, terminal, p_actions)
            ...
            if with_flag and ns[-1]>=self.domain.FlagNum: 
                total_episode_flags += 1
                if total_episode_flags>=30: 
                    self.domain.FlagHeight = self.performance_domain.FlagHeight
                    # print('collect ', total_episode_flags, 'flags')
                    with_flag = False
        ...
        if visualize_steps:
            self.domain.show(a, self.agent.representation)
        self.logger.info("Total Experiment_MountainCar Duration %s" % (hhmmss(deltaT(self.start_time))))



# =============================================================================================================== #
#                class to use when doing the Q-learning with weight inheriting through iterations                 #
# =============================================================================================================== #
class Experiment_ql(Experiment):
    """
    difference from 'Experiment':
    ...
    def __init__(self, agent, domain, performance_domain, exp_id=1, max_steps=1000,
                 ...
                 path='Results/Temp',
                 checks_per_policy=1, stat_bins_per_state_dim=0, **kwargs):
        super(Experiment_ql, self).__init__(agent, domain, performance_domain, exp_id, max_steps,
                                            config_logging, num_policy_checks, log_interval,
                                            path,
                                            checks_per_policy, stat_bins_per_state_dim, **kwargs)
    ...
    def run(self, visualize_performance=0, visualize_learning=False,
        ...
        if visualize_steps:
            self.domain.show(a, self.agent.representation)
        self.logger.info("Total Experiment_ql Duration %s" % (hhmmss(deltaT(self.start_time))))
        return self.agent.representation.weight_vec



# =============================================================================================================== #
#                                    class to use when doing the GridWorld_Items                                  #
#                                       (0 return if didn't get to the goal)                                      #
# =============================================================================================================== #
class Experiment_it(Experiment):

    def __init__(self, agent, domain, performance_domain, exp_id=1, max_steps=1000,
                 config_logging=True, num_policy_checks=10, log_interval=1,
                 path='Results/Temp',
                 checks_per_policy=1, stat_bins_per_state_dim=0, **kwargs):
        super(Experiment_it, self).__init__(agent, domain, performance_domain, exp_id, max_steps,
                                            config_logging, num_policy_checks, log_interval,
                                            path,
                                            checks_per_policy, stat_bins_per_state_dim, **kwargs)
    ...
    def performanceRun(self, total_steps, visualize=False):
        ...
        if not eps_term: 
            eps_return, eps_discount_return = 0, 0
...
# =============================================================================================================== #
#                   class to use when doing the GridWorld_Items & transfer the weight_vec                         #
# =============================================================================================================== #
class Experiment_it_ql(Experiment):

    def __init__(self, agent, domain, performance_domain, exp_id=1, max_steps=1000,
                 config_logging=True, num_policy_checks=10, log_interval=1,
                 path='Results/Temp',
                 checks_per_policy=1, stat_bins_per_state_dim=0, **kwargs):
        super(Experiment_it_ql, self).__init__(agent, domain, performance_domain, exp_id, max_steps,
                                            config_logging, num_policy_checks, log_interval,
                                            path,
                                            checks_per_policy, stat_bins_per_state_dim, **kwargs)
