...
from .define_basic_functions_IS import define_test_pts, obj


class besdGridWorld_(object):
    _num_IS = 2
    num_policy_checks = 1
    checks_per_policy = 1
    all_data = []
    ...
    def __init__(self, prob_name, mult=1.0):
        ...
        if prob_name == 'gw10One1': 
            ...
            self.noise = [0.45, 0.49]
        elif prob_name == 'gw20Three2': 
            self._meanval = 6.778
            self.noise = [0.042, 0.137]
        self.y_min = None
        self.y_max = None
            
    def evaluate(self, IS, x, random_seed, exp_path='./initResults/'):
        max_steps = int(x[0] * self.S)
        if IS==0: repQL = self.repQL
        elif IS==1: repQL = int(self.repQL / 2)
...
class BesdGridWorld(object):
    ...
    def __init__(self, replication_no, method_name, obj_func_idx, bucket, prob_name):
        ...
        self._obj_func = [besdGridWorld_(prob_name, mult=1.0)]
    ...
    @property
    def truth_is(self): return 0 # big r

    @property
    def exploitation_is(self): return 1 # small r

    @property
    def list_sample_is(self): return range(2)
...
class BesdGridWorldMkg(BesdGridWorld):
    ...
    def __init__(self, replication_no, obj_func_idx, bucket, prob_name):
        super(BesdGridWorldMkg, self).__init__(replication_no, "besd", obj_func_idx, bucket, prob_name)

    @property
    def num_is_in(self): return 1   # This should be idx of the last IS
