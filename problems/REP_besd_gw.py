...
class besdGridWorld(object):
    num_policy_checks = 1
    checks_per_policy = 1
    all_data = []
    all_data_raw = []
    
    def __init__(self, prob_name, mult=1.0, flag_num=None):
        ...
        _test_x, _row_num, _col_num, self.maze_num, self.maze_name, \
            ...
            self.s, self.S, self.skip, self.episodeCap = define_test_pts(prob_name, flag_num)
        ...
        if self.flag_num > 2:
            self._search_domain[5,:] = [0, self.maze_size-1]
            self._search_domain[6,:] = [0, self.maze_size-1]
            
        # mean of log(output of QL)
        if prob_name == 'gw10Two1': 
            self._meanval, self.noise = 5.846193004595744, [0.2, 0.3, 0.44]
        elif prob_name == 'gw20Three1': 
            self._meanval, self.noise = 6.555808123675599, [0.33, 0.45, 0.49]
...
class BesdGridWorld(object):
    ...
    def __init__(self, replication_no, method_name, obj_func_idx, bucket, prob_name, flag_num=None):
        self.replication_no = replication_no
        self.method_name = method_name
        self._hist_data = None
        self._bucket = bucket
        self.prob_name = prob_name
        self._obj_func = [besdGridWorld(prob_name, mult=1.0, flag_num=flag_num)]
...
class BesdGridWorldMkg(BesdGridWorld):

    def __init__(self, replication_no, obj_func_idx, bucket, prob_name, flag_num=None):
        super(BesdGridWorldMkg, self).__init__(replication_no, "besd", obj_func_idx, bucket, prob_name, flag_num=flag_num)
