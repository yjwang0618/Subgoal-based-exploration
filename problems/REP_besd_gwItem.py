...
class besdGridWorld_item(object):
    num_policy_checks = 1
    checks_per_policy = 1
    all_data = []
    all_data_raw = []
    ...
    def evaluate(self, repQL, s_x, random_seed, exp_path='./initResults/'):
        ...
        return self._mult*val, y_noise # negate val because we want to maximize rewards
...
class BesdGridWorldItem(object):
    ...
    def __init__(self, replication_no, method_name, obj_func_idx, bucket, prob_name):
        ...
        self._bucket = bucket
        self._obj_func = [besdGridWorld_item(prob_name, mult=-1.0)]
    ...
    @property
    def num_iterations(self): return 70
...
class BesdGridWorldItemMkg(BesdGridWorldItem):

    def __init__(self, replication_no, obj_func_idx, bucket, prob_name):
        super(BesdGridWorldItemMkg, self).__init__(replication_no, "besd", obj_func_idx, bucket, prob_name)



class_collection = {
    "besd_it": BesdGridWorldItemMkg,
}
